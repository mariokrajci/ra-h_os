# USER_GUIDE.md Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create `USER_GUIDE.md` at the repo root — a screen-by-screen, workflow-first guide that lets someone navigate and use RA-H confidently after one read.

**Architecture:** Single Markdown file, seven sections, ordered from orientation → first action → core workflows → reference. Each section is self-contained so readers can jump to what they need.

**Tech Stack:** Markdown, ASCII diagrams, no code dependencies.

**Design doc:** `docs/plans/2026-02-26-user-guide-design.md`

---

### Task 1: Create the file and write Section 1 — The Screen at a Glance

**Files:**
- Create: `USER_GUIDE.md`

**Step 1: Write the file with Section 1**
...
**The core loop:** Click a node anywhere → it opens in the Focus Panel (right side of Slot A, or in Slot B). That's where you read and edit everything about a node.
```

**Step 2: Verify**

Open `USER_GUIDE.md` and confirm:
- ASCII diagram renders correctly
- Toolbar table has all 8 rows
- Slot A/B explanation is clear
- Core loop paragraph is present

---

### Task 2: Write Section 2 — Your First 5 Minutes
...
A modal appears asking for your OpenAI key. If you have one, enter it now. If not, click **"Got it"** — you can still use the app manually; AI features (auto-descriptions, semantic search, auto-tags) just won't work.
...
Confirm section has: 5 numbered steps, code blocks for commands, no branching decisions on the critical path.

---

### Task 3: Write Section 3 — The 5 Core Workflows

**Files:**
- Modify: `USER_GUIDE.md`

**Step 1: Append Section 3 with all five workflows**
...
5. A second input appears: type the relationship in plain English:
   - _"created by"_ → becomes `created_by`
   - _"part of"_ → becomes `part_of`
   - _"source of"_ → becomes `source_of`
   - _"related to"_ → becomes `related_to`
   - Anything else → AI infers the type
6. Press **Enter**

The connection appears immediately. Green arrows = outgoing (from this node). Orange arrows = incoming (to this node). Click any connected node title to jump to it.
...
Confirm each of the 5 workflows:
- Has numbered steps
- Has no unexplained jargon
- Covers what to do when something unexpected happens (spinner, AI inference, etc.)

---

### Task 4: Write Section 4 — Views
...
Confirm: 4 views covered, each has a "best for" use case described.

---

### Task 5: Write Section 5 — Focus Panel Reference
...
Confirm: all 4 tabs covered, drag-to-reference tip included, arrow color meaning explained.

---

### Task 6: Write Section 6 — Settings Reference
...
**API Keys**
Enter your OpenAI API key (`sk-...`) and optionally a Tavily key for web search. Keys are stored in `.env.local` on your machine only. Without an OpenAI key the app works manually — no auto-descriptions, no semantic search, no auto-tags.
...
Confirm: 5 tabs covered, each with one clear paragraph.

---

### Task 7: Write Section 7 — Keyboard Shortcuts
...
Confirm: shortcuts table is complete and accurate, final cross-reference to PROJECT_SUMMARY.md is present.
...
Open `USER_GUIDE.md` and read it top to bottom as if you've never seen the app. Ask:
- Can I find every part of the screen after Section 1?
- Could I complete Workflow 2 (YouTube) from Section 3 alone?
- Is there any jargon that isn't explained before it's used?
- Is the Edges tab explanation in Section 5 consistent with Workflow 4 in Section 3?

**Step 2: Fix any gaps found**

Edit inline — don't add new sections, fix clarity within existing ones.
